...
<div class="list_search_wrap">
	<div class="search_clean">
		<button type="button" class="button" onclick="location.href='<%=cp%>/travel/admin/board/event/list'">새로고침</button>
	</div>
	<form name="search_form" action="<%=cp%>/travel/admin/board/event/list" method="post">
	...
	<tbody>
		<c:forEach var="dto" items="${list}">
			<tr>
				<td>${dto.listNum}</td>
				<td>
					<a class="udline" href="${articleUrl}&eventCode=${dto.eventCode}">
...
<c:if test="${sessionScope.admin.departCode == 4 || sessionScope.admin.departCode == 3}">
	<div class="btn_wrap">
		<p class="f_right"><a href="<%=cp%>/travel/admin/board/event/add" class="button h30 btn_brown w70">등록</a></p>
	</div>
</c:if>