...
<script type="text/javascript">
	$("body").on("click", ".btn_update", function(){
		...
		var linkUrl = "<%=cp%>/travel/admin/board/notice/update${query}&notiCode=${dto.notiCode}";
		
		title.html("공지사항 수정");
		content.html("공지사항 정보를 수정하시겠습니까?");
	...
	$("body").on("click", ".btn_delete", function(){
		...
		var content = $(".pop_wrap .pop_cont p:first-child");
		var link = $(".pop_wrap .pop_cont p.t_center a.btn_blk");
		var linkUrl = "<%=cp%>/travel/admin/board/notice/delete${query}&notiCode=${dto.notiCode}";
		
		title.html("공지사항 삭제");
		content.html("공지사항 정보를 삭제하시겠습니까?");
...
<div class="tbl_btn">
	<p class="f_left">
		<c:if test="${not empty preReadBoard}">
			<a href="<%=cp%>/travel/admin/board/notice/view${query}&notiCode=${preReadBoard.notiCode}" class="button h30 w70 btn_wht">이전</a>
		</c:if>
		<c:if test="${not empty nextReadBoard}">
			<a href="<%=cp%>/travel/admin/board/notice/view${query}&notiCode=${nextReadBoard.notiCode}" class="button h30 w70 btn_wht">다음</a>
		</c:if>
	</p>
	<p class="f_right">
		...
		<a href="<%=cp%>/travel/admin/board/notice/list${query}" class="button h30 w70 btn_blk">목록</a>
	</p>
</div>