<%@ page contentType="text/html; charset=UTF-8"%>
<%@ page trimDirectiveWhitespaces="true" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%
	String cp=request.getContextPath();
%>



<div class="body">
	<div style="height: 50px; margin-left: 20px;">
		<h1>${store.storeName} 티켓 목록</h1>
	</div>
	
	</div>
	<div class="hotel-body" style="width: 100%;">
		<table class="table td_bor_no" style="width: 100%;">
			<thead>
				<tr>
					<th width="70">티켓코드</th>
					<th width="70">티켓명</th>
					<th width="50">인원수</th>
					<th width="50">가격</th>
					<th width="100" colspan="2">관리</th>
				</tr>
			</thead>
			<tbody>
			<c:forEach var="dto" items="${list}">
					<tr>
						<td>${dto.ticketCode}</td>
						<td>${dto.ticektName}</td>
						<td>${dto.hCount}</td>
						<td>${dto.price}</td>
						<td><button type='button' class='button btnUpdateHotel' onclick="location.href='<%=cp%>/store/storeInfo/ticketInfo/updateTicket?TicketCode=${dto.ticketCode}';">티켓 수정</button></td>
						<td><button type='button' class='button btnInsertRoom' onclick="location.href='';">삭제</button></td>
					</tr>
				</c:forEach>
			
			</tbody>
			
		</table>
		<div class="btn_wrap view_btn">
				<button class="button btn" type="button" onclick="location.href='<%=cp%>/hotel/hotelInfo/roomInfo/insertRoom?hotelCode=${hotel.hotelCode}';">객실 추가하기</button>
				<button class="button btn" type="button" onclick="location.href='<%=cp%>/hotel/hotelInfo/list';">호텔 목록으로</button>
		</div>
	</div>