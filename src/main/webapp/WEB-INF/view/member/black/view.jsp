...
<script type="text/javascript">
	...
	$("body").on("click", ".save_confirm", function(){
		var url = "<%=cp%>/member/black/updateBlackCount";
...
<div class="tbl_btn">
	<p class="f_left">
		<c:if test="${not empty preReadUser}">
			<a href="<%=cp%>/member/black/view?${query}&userIdx=${preReadUser.userIdx}" class="button h30 w70 btn_wht">이전</a>
		</c:if>
		<c:if test="${not empty nextReadUser}">
			<a href="<%=cp%>/member/black/view?${query}&userIdx=${nextReadUser.userIdx}" class="button h30 w70 btn_wht">다음</a>
		</c:if>
	</p>
	<p class="f_right">
		<a href="<%=cp%>/member/black/list?${query}" class="button h30 w70 btn_blk">목록</a>
	</p>
</div>
